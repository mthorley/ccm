...
mitigation mappings, and the CCM mapping of TLS policies to 800-53 controls.
...
from .catalog import load_attack_mappings, load_attack_stix, load_catalog, load_csf_linkage, load_policy_mappings, load_sp800_53
from .graph import (
    attack_mapping_statements, attack_statements, csf_statements, ingester_from_environment, linkage_statements,
    mapping_statements, sp800_53_statements,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--mappings", default=str(DEFAULT_MAPPINGS), help="policy -> control mapping YAML; pass '' to skip")
    ...
    mappings = load_policy_mappings(args.mappings, sp800_53) if args.mappings else []
    ...
    try:
        ...
        if mappings:
            ingester.run(mapping_statements(mappings, sp800_53.framework, Path(args.mappings).name))
    finally:
        ingester.close()
