...
flowchart TB
    ...
    fw -- CONTAINS --> fam
    fw -- CONTAINS --> tac
    pol -- "SUPPORTS (tls-policies.yaml)" --> sub
    pol -- "SUPPORTS (tls-policies.yaml, narrow)" --> ctl
    pol -- "MITIGATES (tls-policies.yaml)" --> tech
...
- `mappings/tls-policies.yaml` maps the CEL policy IDs (`CCM-TLS-01`…`04`) in three directions, each pair with a rationale and the item's title as an inline comment: `subcategories:` to the NIST CSF 2.0 subcategories the check evidences (`(:Policy)-[:SUPPORTS]->(:CsfSubcategory)` — `PR.DS-02`, `PR.AA-01`, `PR.PS-01`, `ID.AM-08`); `controls:` to the SP 800-53 controls the check *itself* verifies (`(:Policy)-[:SUPPORTS]->(:Control)` — deliberately narrow: `SC-8`, `SC-8(1)`, `SC-13`, `SC-17`, `SC-23`, nothing that merely relates); and `techniques:` to the ATT&CK techniques it genuinely mitigates (`(:Policy)-[:MITIGATES]->(:Technique)` — `T1557`, `T1040`, `T1565.002`). Nothing reachable only transitively — controls referenced by an evidenced CSF subcategory through OLIR, or techniques CTID maps to an evidenced control — is ever counted as evidence; those links appear only as context in the detail panels. The file is pinned to all three framework versions under `frameworks:`; ingestion rejects unknown ids.
...
(:Policy {id: "CCM-TLS-03"})-[:SUPPORTS {rationale, source}]->(:CsfSubcategory)   # CCM mapping (tls-policies.yaml)
(:Policy {id: "CCM-TLS-03"})-[:SUPPORTS {rationale, source}]->(:Control)          # CCM mapping (tls-policies.yaml, narrow)
(:Policy {id: "CCM-TLS-03"})-[:MITIGATES {rationale, source}]->(:Technique)        # CCM mapping (tls-policies.yaml)
```
...
Mapping files are **pinned** to the version they were validated against (`frameworks: {id: version}` in `tls-policies.yaml`; `source_version` / `target_version` in the linkage file). Ingesting against a different catalog version fails before writing, so a new NIST or MITRE revision never silently inherits the old mapping: bump the version in the file after reviewing it, re-ingest, and the `SUPPORTS` / `REFERENCES` edges into the previous version are retired by the stale-edge prune while the previous version's catalog stays intact.

`Policy` nodes are the same nodes the graph adapter creates from scan events, so one query joins live evidence to CSF (and, through NIST's OLIR references, to the SP 800-53 controls that merely *relate* to it — the controls a policy directly verifies are its own `SUPPORTS` edges):

```cypher
MATCH (e:Endpoint)-[ev:EVALUATED_BY]->(p:Policy)-[:SUPPORTS]->(s:CsfSubcategory {version: "2.0"})<-[:CONTAINS]-(cat:CsfCategory)
OPTIONAL MATCH (s)-[:REFERENCES]->(c:Control)
RETURN cat.id, s.id, p.id, e.fqdn, ev.outcome, collect(c.id) AS related_controls
```

Threat coverage comes from the direct policy → technique edges (not from SP 800-53):

```cypher
MATCH (p:Policy)-[m:MITIGATES]->(t:Technique {version: "16.1"})
RETURN p.id, collect(t.id + ': ' + m.rationale)
```
...
```cypher
MATCH (p:Policy)-[:MITIGATES]->(t:Technique)<-[:USES]-(g:Group)
...
**NIST CSF 2.0** — functions → categories → subcategories, with the evidence ring showing which subcategories the TLS policies reach (via SP 800-53 and NIST's OLIR references):

![Dashboard: NIST CSF 2.0 coverage](docs/images/dashboard-csf.png)

**NIST SP 800-53 r5.2.0** — families → controls. Highlighted controls are ones a TLS policy itself verifies (`controls:` in the mapping — SC-8, SC-8(1), SC-13, SC-17, SC-23); the panel lists the CSF subcategories that reference a control (NIST OLIR) as context. Click a family to zoom (second image: the SC family, SC-8(1) lit on the enhancement ring):

![Dashboard: NIST SP 800-53 coverage](docs/images/dashboard-sp800-53.png)

![Dashboard: SP 800-53 zoomed into the SC family](docs/images/dashboard-sp800-53-zoomed.png)

**MITRE ATT&CK v16.1** — tactics → techniques → sub-techniques (`GET /api/attack`). A technique is lit only when a TLS policy *genuinely* mitigates it, per the `techniques:` section of the hand-maintained `ccm_data_model/mappings/tls-policies.yaml` (today: T1557 Adversary-in-the-Middle, T1040 Network Sniffing, T1565.002 Transmitted Data Manipulation). The transitive path through SP 800-53 is deliberately not used as evidence — CTID maps `SC-8`/`SC-13` to hundreds of techniques a TLS check does nothing about — but the detail panel still lists those CTID controls as context, alongside ATT&CK's own mitigations, the data components that detect the technique, and how many tracked groups use it. Second image: zoomed into Credential Access.

![Dashboard: MITRE ATT&CK coverage](docs/images/dashboard-attack.png)

![Dashboard: ATT&CK zoomed into Credential Access](docs/images/dashboard-attack-zoomed.png)
...
The dashboard's **NIST CSF coverage** section is a csf.tools-style sunburst of the CSF 2.0 core — functions (inner ring), categories, subcategories (outer ring, every subcategory equal-weighted) — with a thin evidence ring outside it. `GET /api/csf` builds it from the graph: for each subcategory, the CCM policies that `SUPPORTS` it directly (from `tls-policies.yaml`, with their rationale), its OLIR-referenced SP 800-53 controls as context, and a status from current endpoint state — `fail`/`warn` if any endpoint's `lastPolicyId` is one of those policies with that outcome, `pass` if endpoints were scanned and none is, `unknown` with no scan data, `unmapped` when no TLS policy reaches the subcategory. Hovering a segment shows the statement, references, policies and per-endpoint outcomes; clicking pins it; a table view lists every subcategory for keyboard and screen-reader use. On the **SP 800-53 wheel** a control is evidenced only through the narrow `controls:` mapping (status rolls up enhancements). The **ATT&CK wheel** uses the same zoomable renderer (tactics → techniques at the root; zoom a tactic for sub-techniques; `#attack/TA0006` or `#attack/TA0006/T1557` deep-link). Because a technique can belong to several tactics it is placed under each, so the wheel has 236 technique placements for 656 live techniques; a technique's status rolls up its sub-techniques, and evidence comes only from the direct `Policy → MITIGATES → Technique` edges written from `tls-policies.yaml` (pinned to the ATT&CK version; unknown technique ids are rejected). A **Zoomable** toggle swaps the static CSF wheel for the same d3 zoomable renderer (three rings; click a function or category to zoom; `#csf/PR` or `#csf/PR.DS` deep-link straight there). The three wheels sit behind a tab bar (**NIST CSF 2.0 · NIST SP 800-53 · MITRE ATT&CK**; arrow keys move between tabs, and `#csf` / `#sp800-53` / `#attack` deep-link to one). The **SP 800-53 wheel**, fed by `GET /api/sp800-53`. It is a d3 **zoomable sunburst** (`ccm_dashboard/sunburst-zoom.js`, d3 v7 from the same CDN as Alpine): at the top level families are the inner ring and the 300 non-withdrawn base controls the outer ring (equal-weighted); clicking a family makes it the centre and reveals its controls with their enhancements as the outer ring; clicking a control shows its enhancements; clicking the centre (or *← all families*) goes back up, with d3's arc-tween transition. Deep links `#sp800-53/SC` and `#sp800-53/SC-8` land directly on a family or control without animating. A control's status rolls up its enhancements (worst of its own and theirs), and the evidence ring always partitions the circle exactly once: it follows the outermost visible ring plus any leaf on the inner ring, so a control without enhancements shows its own status when zoomed into its family. Because 20 families exceed a readable categorical palette, that wheel uses emphasis colouring: neutral alternating fills for structure, one accent for controls with TLS policy evidence, and direct labels only on those controls. Function hues are a validated categorical set (adjacent ring pairs, including the RC→GV wrap, pass colour-vision-deficiency and normal-vision separation checks on the dashboard surface); status is never colour-alone — every status carries an icon and label. Keep raw certificate contents, tokens, and exception text out of span attributes. The persistent queue still depends on the Kubernetes volume and downstream systems eventually recovering; it is not an infinite or transactional guarantee.
